...
viewPreviousGames()
{
	...
	if (selectedGameToReplay == 2)
	{
		...
		if (gameTwoTileTracker[1] != 0)
		{
			...
			puts("Please enter any key to see next move");
			//holds the key input the user enters, this variable is only here to stop the code from executing until a key is entered, the variable has no other purpose
			getchar();
		}
		else
...
void winChecker()
{
	...
	{
		printf("Congratulation %s you have won!", playerone);
		gameRecorder();
	}
	...
	{
		printf("Congratulation %s you have won!", playertwo);
		gameRecorder();
...
void Playermoves()
{
	...
	//used in inputChecker module to check if a tile has already been assigned to
	int checker = 0;
	int counter = 0;
